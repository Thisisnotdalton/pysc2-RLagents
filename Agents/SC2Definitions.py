...
UNIT_TYPES = max(max(UNITS["N"].values()), max(UNITS["T"].values()), max(UNITS["Z"].values()), max(UNITS["P"].values())) + 1 #+1 because of zero indexing
...
def print_race_actions(race):
    print('Actions for {0}:{1}'.format(race, len(ACTIONS[race])))
    for func in ACTIONS[race]:
        print(str(actions.FUNCTIONS[func]))
    print('\n')
    
if __name__ == "__main__":
    classify_actions()
    for race in ACTIONS:
      print_race_actions(race)
